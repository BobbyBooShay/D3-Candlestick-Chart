...
function update_olhc(svg, dataset)
{
    ...
    var group = olhc.append("g").selectAll("g")
    .data(dataset)
    .enter()
    .append("g");
...
{
    //volume
    var volume = svg.append("g").attr("transform", "translate(" + margin.left + "," + (height + margin.top) + ")");
    ...
    var volume_axis = d3.svg.axis().scale(y_scale).orient("left").ticks(3);
    volume.append("g")
    .attr("class", "axis")
    .attr("transform", "translate(" + width + ",0)")
    .call(volume_axis);
